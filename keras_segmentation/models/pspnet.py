...
def pool_block(feats, pool_factor):
    ...
    if IMAGE_ORDERING == 'channels_first':
        h = tf.keras.backend.int_shape(feats)[2]
        w = tf.keras.backend.int_shape(feats)[3]
    elif IMAGE_ORDERING == 'channels_last':
...
def _pspnet(n_classes, encoder,  input_height=384, input_width=576, channels=3):
    ...
    for p in pool_factors:
        ...
        # **CHANGE 1**: Resize each pooled tensor to match `o`'s spatial dimensions
        pooled = Lambda(
            lambda x: tf.image.resize(
                x,
                size=(tf.shape(x)[1], tf.shape(x)[2]),  # Use `x` to dynamically compute shape
                method="bilinear",
            )
        )(pooled)
